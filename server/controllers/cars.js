...
exports.create = async (req, res) => {
    try {
        ...
        if (err.code == 11000) return res.status(400).send({ message: `The car with the licence plate ${req.body.licencePlate} already exists` });
        // Check if there was a missing required attribute 
        else if (err.name === "ValidationError") return res.status(400).send({ message: err.message });
...
exports.get = async (req, res) => {
    try {
        ...
        if (!car) return res.status(404).send({ message: `The car with the id ${carId} was not found` });
        res.send(car);
    } catch (err) {
        console.log(`err ${err}`);
        res.status(500).send({ message: `Something went wrong` });
